...
async fn memory_handler(
    ...
    query: Option<Query<MemoryQuery>>,
) -> Json<serde_json::Value> {
    let limit = query.and_then(|q| q.limit).unwrap_or(10);
...
async fn status_handler(
    ...
    let score = runtime.reputation.route(&did).await;
    let thresholds = runtime.reputation.get_thresholds();
    let status = serde_json::json!({
        "did": did,
        "reputation": score,
        "thresholds": {
            "pro": thresholds.pro,
            "plus": thresholds.plus,
            "standard": thresholds.standard,
            "lite": thresholds.lite,
        },
