...
class ArweaveDataLayer:
    ...
    def upload_data(self, data: Union[str, bytes],
        ...
        """
        # Set up default tags regardless of real vs mock upload
        default_tags = {
            "App-Name": "ARKHE-OS",
            "App-Version": "2.0.0",
            "Substrate": "927",
        ...
        try:
            ...
            if wallet is None:
                raise ImportError("No wallet provided, fallback to mock upload")
...
class PermawebBridge:
    ...
    def spawn_agent_process(self, agent_id: str,
        ...
        if result and "id" in result:
            ...
            # For testing with mocked requests without 'id'
            # Or simulating process spawn
            mock_id = f"mock-ao-{hashlib.sha256(agent_id.encode()).hexdigest()[:8]}"
            self._process_registry[agent_id] = {
                "process_id": mock_id,
                "created_at": datetime.now(timezone.utc).isoformat(),
            }
