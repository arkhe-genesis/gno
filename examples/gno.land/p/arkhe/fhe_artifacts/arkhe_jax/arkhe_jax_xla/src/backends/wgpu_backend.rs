    ...
    fn lower_to_wgsl(&self, graph: &JaxprGraph) -> String {
        ...
        for (prim, _inputs, out) in &graph.tape {
            match prim {
                ...
                Primitive::PhaseShift { theta_bits } => {
                    writeln!(code, "    // PhaseShift({}): node {}", f64::from_bits(*theta_bits), out).unwrap();
                    writeln!(code, "    output[idx] = input_0[idx] * exp(vec2(0.0, {}));", f64::from_bits(*theta_bits)).unwrap();
                }
                _ => {
