...
impl JaxprGraph {
    ...
    pub fn alloc_shape(&mut self, shape: Vec<usize>) -> &[usize] {
        ...
        // SAFETY: The Bump arena guarantees that the data outlives the graph.
        // We launder the lifetime to 'static to decouple the Tracer from a mutable reference to self.
        unsafe { std::mem::transmute::<&[usize], &'static [usize]>(slice) }
    }
